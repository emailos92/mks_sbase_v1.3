...
if build_type == 'upload' or build_type == 'traceback' or build_type == 'Not Set' :
    ...
    if current_OS == 'Linux':
        ...
        target_file_found = False
        target_drive_found = False
        medias = os.listdir('././././output')  #
        for media in medias:
          drives = os.listdir('././././output/' + media)  #
          if target_drive in drives and target_file_found == False:  # set upload if not found target file yet
            target_drive_found = True
            upload_disk = '././././output/' + media + '/' + target_drive + '/'
          for drive in drives:
            try:
              files = os.listdir('././././output/' + media + '/' + drive )
            except:
              continue
            else:
              if target_filename in files:
                if target_file_found == False:
                  upload_disk = '././././output/' + media + '/' + drive + '/'
                  target_file_found = True
